...
nresid = n*(K-1);
nv = K + K*dnew + 1 + 3*nresid + nresid + 1 + (K+1)*d;
...
blk{1,2} = K;
blk{2,1} = 'q';
blk{2,2} = [K*dnew+1, 3*e',(K+1)*ones(1,d)];
blk{3,1} = 'l';
blk{3,2} = nresid + 1;
...
for k = 1:K,
    ...
    A4(rin3+(k-1)*d+2:rin3+k*d+1,(k+1):(K+1):(d-1)*(K+1)+k+1) = -speye(d);
end;
A1(1+2*nresid+1,:) = ones(1,K);
A2(nresid+1,1) = 1;
A3(1:nresid,1:nresid) = speyen;
A3(rin3+1,nresid+1) = 1;
A4(rin3+1,1:(K+1):(d-1)*(K+1)+1) = ones(1,d);
Avec{1,1} = A1';
Avec{2,1} = [A2,A4]';
Avec{3,1} = A3';  %'
%full(A1),
...
meann = n/K;
if weighted_index==0;
    c = zeros(K*dnew+1+3*nresid+(K+1)*d,1);
    ...
    weight = ones(K,1);
    c = ones(K*dnew+1+3*nresid+(K+1)*d,1);
    c1 = ones(nresid,1);
    rind = K*dnew+1;
    cind = 0;
    for k = 1:K,
